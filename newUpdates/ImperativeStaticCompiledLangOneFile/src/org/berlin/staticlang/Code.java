...
import static org.berlin.staticlang.JavaCompilerHelper.ByteCodes.*;

import java.util.ArrayList;
import java.util.List;

import org.berlin.staticlang.JavaCompilerHelper.Flags;
import org.berlin.staticlang.JavaCompilerHelper.Name;
import org.berlin.staticlang.JavaCompilerHelper.Pool;
import org.berlin.staticlang.JavaCompilerHelper.Symbol.MethodSymbol;
import org.berlin.staticlang.JavaCompilerHelper.Symbol.VarSymbol;
import org.berlin.staticlang.JavaCompilerHelper.Type;
import org.berlin.staticlang.JavaCompilerHelper.TypeTags;
...
public class Code {
   ...
   * unsigned shorts.
   */
  List<char[]> lineInfo = new ArrayList<char[]>(); // handled in stack fashion
   ...
   */
  public Code(MethodSymbol meth, boolean fatcode, boolean varDebugInfo,
      StackMapFormat stackMap, boolean debugCode, Pool pool) {
    this.meth = meth;
    this.fatcode = fatcode;    
    this.lineDebugInfo = false;
  ...
  public void resolve(Chain chain, int target) {
    ...
    for (; chain != null; chain = chain.next) {
      ...
      if (get1(chain.pc) == goto_ && chain.pc + 3 == target && target == cp && !fixedPc) {
        ...
        assert !alive || chain.state.stacksize == newState.stacksize && chain.state.nlocks == newState.nlocks;
      }
      fixedPc = true;
      if (cp == target) {
        changed = true;
  ...
  public void resolve(Chain chain) {
    assert !alive || chain == null || state.stacksize == chain.state.stacksize && state.nlocks == chain.state.nlocks;
  ...
  public static Chain mergeChains(Chain chain1, Chain chain2) {
    ...
    if (chain1 == null)
      return chain2;
    assert chain1.state.stacksize == chain2.state.stacksize && chain1.state.nlocks == chain2.state.nlocks;
  ...
  static class LocalVar {
    ...
    public String toString() {
      return "" + sym + " in register " + ((int) reg) + " starts at pc=" + ((int) start_pc) + " length="
          + ((int) length);
    }
  };
  ...
  public void setDefined(Object newDefined) {
  ...
  public void setUndefined(int adr) {
    ...
    if (adr < lvar.length && lvar[adr] != null && lvar[adr].start_pc != Character.MAX_VALUE) {
      LocalVar v = lvar[adr];
